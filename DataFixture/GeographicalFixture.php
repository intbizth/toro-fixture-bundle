...
use Toro\Bundle\AdminBundle\Model\ProvinceInterface;
...
final class GeographicalFixture extends AbstractFixture
{
    ...
    protected function configureOptionsNode(ArrayNodeDefinition $optionsNode)
    {
        ...
        $provinceNode
            ...
            ->normalizeKeys(false)
            ->useAttributeAsKey('code')
            ->prototype('array')->prototype('scalar')
        ;
    ...
    private function loadProvincesForCountry(array $provinces, CountryInterface $country)
    {
        foreach ($provinces as $provinceCode => $provinceName) {
            ...
            foreach ($provinceName as $locale => $name) {
                $province->setCurrentLocale($locale);
                $province->setFallbackLocale($locale);
